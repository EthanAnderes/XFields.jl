...
struct S0S2map{FT<:rFourierTransform} <: XField{FT}
    ...
    S0S2map{FT}(n::Number) where {FT<:rFT2} = new{FT}(fill(F64(n), Grid(FT).nxi,3))
...
struct S0S2fourier{FT<:rFourierTransform} <: XField{FT}
    ...
    S0S2fourier{FT}(n::Number) where {FT<:rFT2} = new{FT}(fill(CF64(n), Grid(FT).nxi,3))
    S0S2fourier{FT}() where {FT<:rFT2}          = new{FT}(zeros(CF64, Grid(FT).nxi,3))
end
